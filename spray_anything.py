...
def load_spray_map(base_dir, h, w):
    """Load the SprayMap_composite.png displacement map."""
    path = os.path.join(base_dir, "SprayMap_composite.png")
    if not os.path.exists(path):
        print(f"ERROR: SprayMap_composite.png not found in {base_dir}")
        sys.exit(1)

    spray = cv2.imread(path)
